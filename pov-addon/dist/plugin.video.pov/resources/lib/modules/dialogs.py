...
def video_playlist_choice(params):
	from caches import video_playlists_cache as playlists
	choices = [(i['name'], '%s (%s)' % (i['name'], i['count'])) for i in playlists.list_names()]
	choices.append(('__new__', 'Create new playlist'))
	list_items = [{'line1': i[1], 'icon': media_path('lists.png')} for i in choices]
	choice = select_dialog([i[0] for i in choices], items=json.dumps(list_items), heading='Add to Video Playlist')
	if choice is None: return
	if choice == '__new__':
		choice = kodi_utils.dialog.input('Playlist name').strip()
		if not choice: return
	mediatype = params.get('content') or params.get('mediatype')
	title = params.get('title') or params.get('show_title') or params.get('name') or ''
	if mediatype == 'season' and params.get('season'):
		title = '%s - Season %s' % (title, params.get('season')) if title else 'Season %s' % params.get('season')
	elif mediatype == 'episode' and params.get('season') and params.get('episode'):
		title = '%s - %sx%s' % (title, params.get('season'), str(params.get('episode')).zfill(2)) if title else '%sx%s' % (params.get('season'), str(params.get('episode')).zfill(2))
	if playlists.add_item(choice, mediatype, params.get('tmdb_id'), title, params.get('season'), params.get('episode')):
		return notification('Added to %s' % choice, 1500)
